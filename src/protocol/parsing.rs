...
fn read_varint(input: &[u8]) -> nom::IResult<&[u8], i32> {
    ...
    for i in until_stop.iter().chain(stop_byte.iter()).copied() {
...
#[allow(dead_code)]
fn read_short(input: &[u8]) -> nom::IResult<&[u8], u16> {
...
fn read_string(input: &[u8]) -> nom::IResult<&[u8], &str> {
...
fn write_packet(id: i32, data: &[u8]) -> Vec<u8> {
    ...
    tracing::debug!(
        packet_length,
        "packet length (no packet length field included)"
    );

    let packet_length_len = varint_len(
        packet_length
            .try_into()
            .expect("len is bigger than i32::MAX"),
    );
    ...
    write_varint(
        ...
        packet_length
            .try_into()
            .expect("Packet length bigger than i32::MAX"),
    );
...
fn read_packet(input: &[u8]) -> nom::IResult<&[u8], (i32, &[u8])> {
    ...
    nom::IResult::Ok((input, (packet_id, data)))
}
...
pub fn parse_status_response(buf: &[u8]) -> Result<(usize, StatusResponse), ParseError> {
    ...
    Ok((consumed, StatusResponse { json_response }))
}
...
mod test {
    ...
    fn test_var_parse_serialize() {
        let test_vectors = [764, -1, 80, 25565, 1];
